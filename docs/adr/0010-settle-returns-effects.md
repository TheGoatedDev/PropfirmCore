...
The ingest worker persists breach rows, then calls `bridge.closePosition`. Loopback closePosition is a no-op. A local close without the broker would come back on the next snapshot.
