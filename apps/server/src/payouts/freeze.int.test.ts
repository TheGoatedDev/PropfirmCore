...
it("freeze refuses fills until approve then withdraws", {
    timeout: 30_000,
}, async () => {
    const email = `fz${Date.now()}@example.com`;
    ...
    const fills = await post(
        ...
        { fills: days.map((ts, i) => fill(ts, `fz-f-${Date.now()}-${i}`)) },
        ingest,
    );
    ...
    const frozen = await post(
        `/ingest/trading-accounts/${id}/fills`,
        { fills: [fill("2026-01-16T17:00:00.000Z", `fz-block-${Date.now()}`)] },
        ingest,
    );
    expect(frozen.status).toBe(409);

    const stillSnap = await post(
        `/ingest/trading-accounts/${id}/snapshot`,
        {
            externalId: `fz-pending-${Date.now()}`,
            equity: 53000,
            balance: 53000,
            ts: "2026-01-16T17:00:00.000Z",
            positions: [],
        },
        ingest,
    );
    expect(stillSnap.status).toBe(202);
    ...
    const unfrozen = await post(
        `/ingest/trading-accounts/${id}/fills`,
        { fills: [fill("2026-01-16T18:00:00.000Z", `fz-ok-${Date.now()}`)] },
        ingest,
    );
    expect(unfrozen.status).toBe(202);
});
