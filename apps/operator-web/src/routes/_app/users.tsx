...
const userSearch = {
    ...
    kind: parseAsStringLiteral(["operator", "firmUser"]),
};
...
function Users() {
    ...
    const [{ q, page, sort, order, role, banned, kind }, setSearch] =
        useQueryStates(userSearch);
    const [filter, setFilter] = useState(q);
    const [creating, setCreating] = useState(false);
    const [createKind, setCreateKind] = useState<"operator" | "firmUser">(
        "firmUser",
    );
    ...
    const create = useMutation({
        mutationFn: async (input: z.infer<typeof createSchema>) => {
            const { error } = await api.POST("/users", { body: input });
            if (error) throw error;
        },
        onSuccess: async () => {
            setCreating(false);
    ...
    return (
        <section className="space-y-3">
                        ...
                        >
                            <div className="space-y-1">
                                ...
                                <select
                                    ...
                                    data-testid="user-create-kind"
                                    value={createKind}
                                    onChange={(ev) =>
                                        setCreateKind(
                                            ev.target.value === "operator"
                                                ? "operator"
                                                : "firmUser",
                                        )
                                    }
            ...
            <div className="flex flex-wrap gap-2">
                <select
                    className={selectClass}
                    data-testid="user-filter-kind"
                    value={kind ?? ""}
                    onChange={(ev) => {
                        const v = ev.target.value;
                        void setSearch({
                            kind:
                                v === "operator" || v === "firmUser" ? v : null,
                            page: 0,
                        });
                    }}
                >
